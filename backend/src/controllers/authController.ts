...
export const register = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const user = await prisma.user.create({
      data: { name, email, password: hashedPassword, role: 'USER', phone: phone ?? null, address: address ?? null },
    });
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (user.isBlocked) { res.status(403).json({ error: 'Your account has been blocked. Please contact support.' }); return; }
...
export const getMe = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const user = await prisma.user.findUnique({
      where:  { id: userId },
      select: { id: true, name: true, email: true, role: true, phone: true, address: true, avatarUrl: true, licenseUrl: true, licenseVerified: true, isBlocked: true },
    });
...
export const updateProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const user = await prisma.user.update({
      where:  { id: userId },
      data,
      select: { id: true, name: true, email: true, role: true, phone: true, address: true, avatarUrl: true, licenseUrl: true, licenseVerified: true },
    });
...
export const uploadLicense = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!licenseUrl) { res.status(400).json({ error: 'licenseUrl is required' }); return; }
...
export const changePassword = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!isMatch) { res.status(400).json({ error: 'Incorrect current password' }); return; }
