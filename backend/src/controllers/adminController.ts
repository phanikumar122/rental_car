...
export const getStats = async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const revenue = revenueData.reduce((s: number, b: { totalAmount: number }) => s + b.totalAmount, 0);
...
export const blockUser = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const user = await prisma.user.update({ where: { id: id as string }, data: { isBlocked: true }, select: { id: true, name: true, isBlocked: true } });
    res.json(user);
  } catch (error) { console.error('[blockUser]', error); res.status(500).json({ error: 'Failed' }); }
};
...
export const unblockUser = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const user = await prisma.user.update({ where: { id: id as string }, data: { isBlocked: false }, select: { id: true, name: true, isBlocked: true } });
    res.json(user);
  } catch (error) { console.error('[unblockUser]', error); res.status(500).json({ error: 'Failed' }); }
};
...
export const verifyLicense = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const user = await prisma.user.update({ where: { id: id as string }, data: { licenseVerified: true }, select: { id: true, name: true, licenseVerified: true } });
    res.json(user);
  } catch (error) { console.error('[verifyLicense]', error); res.status(500).json({ error: 'Failed' }); }
};

/* ─── POST /api/admin/notifications/send ────────────────────────────────────── */
export const sendManualNotification = async (req: Request, res: Response): Promise<void> => {
  try {
    const { userId, channel, message } = req.body as { userId: string; channel: 'EMAIL' | 'SMS' | 'WHATSAPP'; message: string };
    ...
    res.json({ success });
  } catch (error) { console.error('[sendManualNotification]', error); res.status(500).json({ error: 'Failed' }); }
};
...
export const getNotifications = async (req: Request, res: Response): Promise<void> => {
  ...
  } catch (error) { console.error('[getNotifications]', error); res.status(500).json({ error: 'Failed' }); }
};
